...
class MEMO(BaseLearner):
    ...
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self.args["init_epoch"] = args["memo_init_epoch"]
        self.args["init_lr"] = args["memo_init_lr"]
        self.args["init_weight_decay"] = args["memo_init_weight_decay"]
        self.args["init_lr_decay"] = args["memo_init_lr_decay"]
        self.args["lrate"] = args["memo_lrate"]
        self.args["batch_size"] = args["memo_batch_size"]
        self.args["weight_decay"] = args["memo_weight_decay"]
        self.args["lrate_decay"] = args["memo_lrate_decay"]
        self.args["epochs"] = args["memo_epochs"]
        self.args["T"] = args["memo_T"]
        # self.args["model_name"] = args["model_name"]
    ...
    def _cl_compute_accuracy(self, model, loader, device):
        model.eval()
    ...
    def _adv_compute_accuracy(self, model, loader, device):
        model.eval()
        model = model.to(device)
        correct, total = 0, 0
        # test = False
        with torch.no_grad():
            for i, (inputs, targets) in enumerate(loader):
                inputs = inputs.to(device)
                # print("==> inputs:", inputs.shape, "\t Target:", targets.shape)
                with torch.no_grad():
                    # print("===>", type(model(inputs)), model(inputs))
                    outputs = model(inputs)["logits"]
                    predicts = torch.max(outputs, dim=1)[1]
    ...
    def save_memo_checkpoint(self, w_path, test_acc):
        logging.info("Model_name={}".format(self.args['model_name']))
        # assert self.args['model_name'] == 'finetune'
        # checkpoint_name = f"checkpoints/finetune_{self.args['csv_name']}"
        _checkpoint_cpu = copy.deepcopy(self._network)
        if isinstance(_checkpoint_cpu, nn.DataParallel):
