...
class DataManager(object):
    ...
    def get_dataset(
        ...
        for idx in indices:
            ...
            # logging.info(
            #     "==> get_dataset:Input ({}, {}) # {}".format(class_data[0].min().item(), class_data[0].max().item(), len(class_data)))
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(self.trsf)
        # logging.info("==> DummyDataset:Input ({}, {}) # {}".format(image[0].min().item(), image[0].max().item(), len(image)))
