...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self._adv_train_flag = args["adv_train"]  # Added by Hikmat
    ...
    def update_fc(self, nb_classes, nextperiod_initialization=None):
        ...
        if self.fc is not None:
            ...
            if nextperiod_initialization is not None:
                ...
                if self._args["model_name"] == "simplecil":
                    print("==>SIMPLECIL")
                    weight = torch.cat([weight, torch.zeros(nb_classes - nb_output, self.feature_dim).to(self._args["device"][0])])
                else:
...
class AdaptiveNet(nn.Module):
    ...
    def _x_embedding(self, model, loader, device):
        model.eval()
        model.to(device)
        X = None
        Y = None
        with torch.no_grad():
            for i, (x, y) in enumerate(loader):
                print("--->  X:", x.shape, "\tY:", torch.unique(y))
